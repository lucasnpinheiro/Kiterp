            ...
            if (count($empresas) > 0) {
                foreach ($empresas as $key => $value) {
                    ...
                    <div role="tabpanel" class="tab-pane <?php echo ($key > 0 ? '' : 'active'); ?>" id="empresa_<?php echo $value->id; ?>">
                        ...
                        echo $this->Form->impostos('ProdutosValor.' . $value->id . '.cst_cofins', ['value' => prde($lista_produtos_valores, $value->id, 'cst_cofins'), 'div' => ['class' => 'col-xs-12 col-md-2']], 5);
                        echo $this->Form->impostos('ProdutosValor.' . $value->id . '.cst_icms', ['value' => prde($lista_produtos_valores, $value->id, 'cst_icms'), 'div' => ['class' => 'col-xs-12 col-md-2']], 1);
                        echo $this->Form->impostos('ProdutosValor.' . $value->id . '.cst_origem', ['value' => prde($lista_produtos_valores, $value->id, 'cst_origem'), 'div' => ['class' => 'col-xs-12 col-md-2']], 6);
                        echo $this->Form->impostos('ProdutosValor.' . $value->id . '.origem', ['value' => prde($lista_produtos_valores, $value->id, 'origem'), 'div' => ['class' => 'col-xs-12 col-md-2']], 8);
