...
class ContasPagarController extends AppController {
    ...
    public function index() {
        ...
        $query = $this->{$this->modelClass}->find('search', $this->{$this->modelClass}->filterParams($this->request->query))->order(['ContasPagar.status' => 'asc', 'ContasPagar.data_vencimento' => 'desc'])->contain(['Empresas' => function($q) {
                return $q->contain('Pessoas');
            }, 'Pessoas', 'Bancos']);
        $this->set('contasPagar', $this->paginate($query));
        $this->set('_serialize', ['contasPagar']);
        $pessoas = $this->ContasPagar->Pessoas->find('list')->contain(['PessoasAssociacoes' => function($q) {
                return $q->where(['tipo_associacao' => 3, 'status' => 1]);
            }]);
